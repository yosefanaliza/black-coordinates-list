...
class CoordinatesService:
    """Service for resolving IP addresses to geographic coordinates"""
    ...
    async def call_external_ip_api(self, ip: IPField) -> Optional[Dict[str, Any]]:
        """
        ...
        """
        url = f"{self.IP_API_BASE_URL}/{ip}"

        for attempt in range(self.MAX_RETRIES):
            try:
                async with httpx.AsyncClient() as client:
                    logger.info(f"Calling external API for IP: {ip} (attempt {attempt + 1}/{self.MAX_RETRIES})")
                    response = await client.get(url, timeout=self.IP_API_TIMEOUT)
                ...
                logger.warning(f"Timeout calling external API for {ip} (attempt {attempt + 1}/{self.MAX_RETRIES})")
    ...
    async def forward_to_service_b(self, data: CoordinateItem) -> bool:
        """
        Forward coordinate data to Service B for storage

        Args:
            data: CoordinateItem containing IP and coordinates
    ...
    async def resolve_ip(self, ip: IPField) -> CoordinateStorageResponse:
        """
        ...
        geo_data = await self.call_external_ip_api(ip)
        ...
        forward_success = await self.forward_to_service_b(payload)
